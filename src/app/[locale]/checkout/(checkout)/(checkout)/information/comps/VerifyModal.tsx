...
type VerifyModalProps = {
  ...
  onVerify?: () => void;
  resetDefault?: (addressId: string) => void;
...
export default function VerifyModal({
  ...
  const { setGuestVerifiedPhones } = useOrderContext();

  const { mutate: verifyPhoneNumberGuest } = nextApi.order.verifyPhoneNumber.useMutation({
    onSuccess: (data) => {
      setIsVerify(data.verified);
  ...
  const { mutate: verifyPhoneNumberUser } = nextApi.address.verifyPhoneNumber.useMutation({
    onSuccess: (data) => {
      setIsVerify(data.verified);
      setTimeout(onVerifySuccess, DELAY_AUTO_CLOSE);
    },
  ...
  const setVerifiedPhoneToContext = () => {
  ...
  const onCloseModal = () => {
    ...
    if (isVerify) {
      ...
      setVerifiedPhoneToContext();
    }
  };

  const onResend = () => {
    api.order.sendVerifyPhone.mutate({ phoneNumber, countryId });
        ...
        <Switch.Root>
          ...
          <Switch.Case when={isVerify}>
            <div className="relative h-auto w-full bg-yel-25 p-6 md:px-20 md:py-14">
              <button onClick={onCloseModal} className="absolute end-4 top-4 md:end-10 md:top-10">
