...
//RELATIVE MODULES
import { Arrow, ButtonList, Dot, sliderClasses } from './slide-section-common';

type Props = {
  list: Extract<
    RouterOutputs['siteContent']['getSiteContentByComponent'],
    { status: true }
  >['data'];
};

const SlideComponent = ({ list }: Props) => {
  ...
  return (
    <>
      ...
      >
        {list.map(({ title, subtitle, mediaUrl }) => {
          return (
            <div
              key={title}
              className={classcat([
            ...
            >
              <div
                ...
                {...(mediaUrl
                  ? {
                      style: {
                        backgroundImage: `url(${process.env.NEXT_PUBLIC_CDN_HOST + mediaUrl})`,
                      },
                    }
                  : {})}
              />
