...
export default function InfoContent({ dish }: Props) {
  if (!dish) return null;
  return (
    <div className={classcat(['flex flex-col space-y-6 px-6', 'lg:px-[unset]'])}>
      ...
      <div className="space-y-2">
        <div className={classcat(['flex flex-row items-center justify-between'])}>
          ...
          <TooltipRadix delayDuration={300} description="Some brief explanation">
            <span>
...
function NutriScoreComp({ value }: { value: NutriScore }) {
  return (
    <div>
      <div className={classcat(['inline-flex flex-row-reverse items-center pb-6'])}>
