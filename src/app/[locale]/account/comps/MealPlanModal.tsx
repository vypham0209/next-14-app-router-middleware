...
type TMealPlanModalProps = {
  id: string;
};

function MealPlanModal({ children, id }: PropsWithChildren<TMealPlanModalProps>) {
  const { data } = nextApi.mealPlan.getMealPlanOrderDetail.useQuery({
    mealPlanOrderId: id,
  });
  return (
    <Modal.Root>
      ...
      <Modal.Portal>
        ...
        <div className="fixed z-toast flex items-end justify-start dir-inset-0 md:items-center md:justify-center">
          ...
          >
            ...
            >
              <Modal.Title className="text-36 text-blu-400 md:text-48">My menu</Modal.Title>
              {data?.data && <MealMenuTab data={data?.data} />}
            </div>
          </Modal.Content>
        </div>
      </Modal.Portal>
